...
const insuranceTest: NightwatchTests = {
  ...
  '1. Landing on the Ornikar insurance auto page test': (browser: NightwatchAPI) => {
    const insurancePage = browser.page.InsuranceAuto.InsurancePage();

    insurancePage
      .assert.titleEquals('Assurance auto : Devis et simulation en ligne gratuits')
      .assert.visible('@insuranceDropdown')
  },
  
  '2. Liscense plate input form validation test - invalid entries': (browser: NightwatchAPI) => {
    const insurancePage = browser.page.InsuranceAuto.InsurancePage();
    insurancePage.closeConsentPopup();
    insurancePage
      .waitForElementVisible('@insuranceDropdown')
